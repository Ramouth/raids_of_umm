...
| 17g | **Shooters (HoMM3)**: an archer with an enemy next to it cannot shoot, it fights hand to hand. **Readied shot** (fire once a round at an enemy moving closer, before its strike) is an advanced ability: Marksmen's Tower (needs a Citadel); guards can be given it | ✅ |
...
| 20 | **HoMM3 map pop-ups** (for the true feel; optional, later): visiting a site opens a small framed box with the site's picture, the message and OK; choices (chest: gold or experience) as buttons in the same box; replaces the notice line for site events | ⬜ later |
