...
GLuint loadTexturePNG(const std::string& path) {
    ...
    // Decompress all concatenated IDAT chunks (one zlib stream)
    ...
    for (uint32_t y = 0; y < h; ++y) {
        ...
        for (uint32_t x = 0; x < w; ++x) {
            pixels[(y * w + x) * 4 + 0] = row[x * bpp + 0];
            pixels[(y * w + x) * 4 + 1] = row[x * bpp + 1];
            pixels[(y * w + x) * 4 + 2] = row[x * bpp + 2];
            pixels[(y * w + x) * 4 + 3] = (bpp == 4) ? row[x * bpp + 3] : 255u;
        }
        prev = row;
    }
